...
class CaravanController extends Controller
{
    ...
    public function update_caravan(Request $request)
    {
        $request->validate([
            'meta_title'=> 'nullable',
        ...
        $info5= Info::where('key','caravan_meta_title')->first();
        if(!$info5){
            $info5 = new Info;
        }
        $info5->key = 'caravan_meta_title';        
        $info5->value = serialize($request->meta_title);
        $info5->save();

        $info4= Info::where('key','caravan_meta_description')->first();
        if(!$info4){
            $info4 = new Info;
        }
        $info4->key = 'caravan_meta_description';        
